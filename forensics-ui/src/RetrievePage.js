...
function RetrievePage() {
  ...
  const handleFetchEvidence = async (e) => {
    e.preventDefault();
    if (!txHash) {
      setStatus('Error: Please enter a transaction hash');
      return;
    }
    setStatus('Fetching evidence ID...');
    try {
      const response = await axios.post('http://localhost:5000/fetch-evidence', { tx_hash: txHash });
      console.log('Fetch evidence response:', response.data); // Debug
      if (response.data.error) {
        setStatus(`Error: ${response.data.error}`);
      } else {
        setEvidenceId(response.data.evidence_id.toString());
        setStatus('Success: Evidence ID populated');
      }
    } catch (error) {
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      {retrievedEvidence && retrievedEvidence.data && (
        <div className="mt-6 p-4 bg-white border rounded-lg shadow-sm">
          ...
          <p className="mb-2"><strong>Post ID:</strong> {retrievedEvidence.data.id || retrievedEvidence.data[0]}</p>
          <p className="mb-2"><strong>Text:</strong> {retrievedEvidence.data.text || retrievedEvidence.data[1]}</p>
          <p className="mb-2"><strong>Author:</strong> {retrievedEvidence.data.author_username || retrievedEvidence.data[2]}</p>
          {retrievedEvidence.data.media_urls?.length > 0 && (
            <div className="mb-2">
              <strong>Media URLs:</strong>
              <ul className="list-disc pl-5">
                {retrievedEvidence.data.media_urls.map((url, i) => (
                  <li key={i}>
